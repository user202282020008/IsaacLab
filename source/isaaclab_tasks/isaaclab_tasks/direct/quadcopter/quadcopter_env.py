...
class QuadcopterEnvCfg(DirectRLEnvCfg):
    ...
    decimation = 2
    action_space = 4             # [推力比, 滚转目标速度, 俯仰目标速度, 偏航目标速度]
    ...
    # --- 底层飞控 PD 参数 (角速度闭环) ---
    max_body_rate = 3.14         # 最大角速度 (rad/s)
    rate_kp = 0.05               # 比例增益 (P)
    rate_kd = 0.005              # 微分增益 (D)
...
class QuadcopterEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: QuadcopterEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 核心：记录上一帧的角速度误差，用于计算微分项
        self._rate_error_last = torch.zeros(self.num_envs, 3, device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # 2. 底层飞控：角速度 PD 计算
        desired_rates = self._actions[:, 1:] * self.cfg.max_body_rate
        current_rates = self._robot.data.root_ang_vel_b
        
        rate_error = desired_rates - current_rates
        rate_error_dot = (rate_error - self._rate_error_last) / self.step_dt
        
        self._moment[:, 0, :] = (self.cfg.rate_kp * rate_error) + (self.cfg.rate_kd * rate_error_dot)
        self._rate_error_last = rate_error.clone()
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # 核心：重置时清空坠毁无人机的 PD 误差历史
        self._rate_error_last[env_ids] = 0.0
